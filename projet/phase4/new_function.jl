...
function neighbord_node(G::Graph{T}, node::Node{T}) where T
    ...
    if (name(nod), name(node)) in keys(weights)&& in_edges(edges(G), Edge((node, nod), weights[(name(node), name(nod))]))]
        succ
    end
...
"""Créee un cycle apartir d'un 1-arbre de racine 'root' et d'un dictionnaire
   de poids des aretes 'weights', en utilisant l'arête de poids minimal
   partant de root
"""
